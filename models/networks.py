...
#              imageSize, ngpu, nz,
#              norm='none', use_dropout=False, init_type='normal', gpu_ids=[]):
def define_G(input_nc, output_nc, ngf, which_model_netG,
            imageSize, ngpu, nz,
    ...
    if which_model_netG == 'VAE':
        netG = VAEGenerator(input_nc, ngf, imageSize, ngpu, nz)
    else:
...
def define_D(input_nc, ndf, which_model_netD,
             imageSize, ngpu, ngf,                     #VAE-GAN parameters
             init_type='normal', gpu_ids=[]):
    netD = None
    #norm_layer = get_norm_layer(norm_type=norm)
    ...
    if which_model_netD == 'VAE':
        netD = VAE_netD(input_nc, ndf, imageSize, ngpu, ngf)
    else:
...
class VAE_netD(nn.Module):
    def __init__(self, nc, ndf, imageSize, ngpu, ngf):
        super(VAE_netD, self).__init__()
        self.ngpu = ngpu
        n = math.log2(imageSize)
        print(imageSize)
        print(n)
        ...
        for i in range(n - 3):
            self.main.add_module('conv'.format(ngf * 2 ** (i), ngf * 2 ** (i + 1)),
                                 nn.Conv2d(ndf * 2 ** (i), ndf * 2 ** (i + 1), 4, 2, 1, bias=False))
            self.main.add_module('batchnorm'.format(ngf * 2 ** (i + 1)), nn.BatchNorm2d(ndf * 2 ** (i + 1)))
            self.main.add_module('relu'.format(ngf * 2 ** (i + 1)), nn.LeakyReLU(0.2, inplace=True))
